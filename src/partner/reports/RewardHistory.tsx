...
// import { useRewardHistory } from "./hooks/useReportClient";
// import { exportRewardsCSV, exportRewardsXLSX, exportRewardsPDF } from "./utils/reportClientExport";
import { useDebounce } from "@/hooks/useDebounce";
import { useRewardHistory } from "@/features/partner/reports/hooks/reports.hooks";
import { exportRewardsCSV, exportRewardsPDF, exportRewardsXLSX } from "@/utils/exportUtils";
...
export default function RewardHistory() {
  ...
  const { data, isLoading, isFetching } = useRewardHistory({
  ...
  return (
    <div className="w-full space-y-8 text-black dark:text-white">
        ...
        <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row">
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-300 bg-white px-10 py-2 text-sm focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:bg-slate-900 dark:text-white"
            />
          </div>
          <div className="relative w-full md:w-72">
            <FiSearch className="absolute top-3 left-3 text-gray-400" />
        ...
        <div className="relative" ref={exportRef}>
          ...
          {exportOpen && (
            <motion.div
              ...
              className="absolute right-0 z-20 mt-2 w-40 rounded-md border bg-white py-2 text-sm shadow-lg dark:border-gray-700 dark:bg-slate-800"
            >
              {[
                {
                  label: "PDF",
                  fn: () => exportRewardsPDF(details),
                  icon: <PiFilePdf size={16} />,
                },
                {
                  label: "CSV",
                  fn: () => exportRewardsCSV(details),
                  icon: <PiFileCsv size={16} />,
                },
                {
                  label: "Excel",
                  fn: () => exportRewardsXLSX(details),
      ...
      {/* Table */}
      <div className="overflow-auto rounded-md border bg-white dark:border-gray-700 dark:bg-slate-800">
          ...
          <thead>
            <tr className="bg-gray-100 dark:bg-slate-700">
              {TABLE_HEADS.map((h) => (
                <th
                  key={h}
                  className="border px-4 py-3 text-center font-medium whitespace-nowrap dark:border-gray-700"
                >
                  {h}
                </th>
              ))}
      ...
      <div className="flex flex-col items-center justify-between gap-4 py-2 md:flex-row">
        <div className="flex items-center gap-2 text-sm">
          ...
          <select
            ...
            className="rounded-md border border-gray-300 bg-white px-2 py-1 dark:border-gray-700 dark:bg-slate-900 dark:text-white"
          >
            {ROWS_OPTIONS.map((n) => (
        ...
        <div className="flex items-center gap-2 text-sm">
          <button
            ...
            className="disabled:opacity-40 dark:disabled:text-gray-600"
          >
            Prev
          </button>
